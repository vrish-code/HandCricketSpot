        ...
        with st.container(border=True):
         ...
         with st.container(border=True):
            ...
            with c1:
                ...
                st.write(f"Player to Bowl")
                st.warning("Please don't use this playing system after you got the wicket or after the match is over.")
                runPlayed=st.slider("Choose what to play.", min_value=1, max_value=11)
                play=st.button(f"Play {runPlayed}?")
                if play:
                   st.session_state.playDict["Balls bowled"].append(runPlayed)
                   st.session_state.playDict["Balls played"]-=1


            with c2:
                st.badge("Bot", color="green")
                st.write(f"Bot to Bat")
                if play: 
